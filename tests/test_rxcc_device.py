...
class RxccDeviceTest(unittest.TestCase):
    def test_gpio_report_matches_reference_shape(self) -> None:
        self.assertEqual(build_gpio_report(3, 1), bytes([0x0F, 0x14, 0x00, 0x02, 0x03, 0x01]))
    ...
    def test_frontend_mode_reports_match_documented_sequence(self) -> None:
        self.assertEqual(
            frontend_mode_reports(FrontendMode.TRANSMITTING_PA),
            [
                bytes([0x0F, 0x14, 0x00, 0x02, 0x00, 0x01]),
                bytes([0x0F, 0x14, 0x00, 0x02, 0x01, 0x00]),
                bytes([0x0F, 0x14, 0x00, 0x02, 0x02, 0x00]),
            ],
        )
    ...
    def test_antenna_report_matches_documented_sequence(self) -> None:
        self.assertEqual(
            ...
            [bytes([0x0F, 0x14, 0x00, 0x02, 0x03, 0x01])],
        )
    ...
    def test_start_rf_enforces_mode_then_antenna_then_start(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                ...
                bytes([0x0F, 0x14, 0x00, 0x02, 0x01, 0x00]),
                bytes([0x0F, 0x14, 0x00, 0x02, 0x02, 0x00]),
                bytes([0x0F, 0x14, 0x00, 0x02, 0x03, 0x00]),
